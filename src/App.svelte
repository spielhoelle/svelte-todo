<script>
  ...
  let filterByWhat = "";
  let isLoading = true;
  let isAuthenticated = localStorage.getItem("svelte-pouch-couchdb");
  ...
  async function login() {
    if(password === "123"){
      localStorage.setItem("svelte-pouch-couchdb", Date.now())
      await updateTodos();
  ...
  async function logout() {
    localStorage.removeItem("svelte-pouch-couchdb")
...
{#if isAuthenticated}
  <div>
    ...
    <button class="float-right" on:click={logout}>Logout</button>
  </div>
  <div>
    <label>Filter:</label>
...
{:else}
  ...
  <form on:submit|preventDefault={login}>
    <input autofocus="true" type="password" bind:value={password} />
